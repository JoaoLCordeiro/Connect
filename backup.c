...
{
    if ((tab->m[l_ori][c_ori]->tipo == MEXE) || (tab->m[t_ori][c_ori]->tipo == MEXEGIRA))
    {
...
void imprime_tabuleiro (WINDOW *tab, t_tabuleiro *tab)
{
    int i,j,k;
    char c;
    wclear(tab);
    for (i=0 ; i<4 ; i++)
    {
        for (j=0 ; j<4 ; j++)
        {
            if (tab->m[i][j]->tipo != NADA)                     //verifica se tem peça
            {
...
int main ()
{
    ...
    WINDOW *wtabuleiro = newwin (25,41,0,0);
